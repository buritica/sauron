...
1. **Metrics + Logs + Traces, unified:** Cloud Portal → your stack → Connections → search "OpenTelemetry" / "Alloy" (infrastructure "Other") → Instrumentation Hub's "Use an API Token" step generates ONE Access Policy token (`metrics:write` + `logs:write` + `traces:write` scopes) and shows the OTLP gateway endpoint + a Basic-auth header in one go — decode that header (`base64 -d`, format is `instanceID:token`) to get the instance ID and token separately. This replaces the old per-product Prometheus/Loki "Send data" pages (separate URLs + instance IDs per signal) — one gateway, one credential, covers everything below AND ca's own direct OTLP push (see below).
2. `echo -n 'YOUR_TOKEN' > config/alloy/grafana-cloud-password` (gitignored, never commit) — on halfmoon, not in this repo checkout.
3. Uncomment the ENTIRE pipeline in `config/alloy/config.alloy` (it's commented out as one block deliberately — see its header), fill in `otelcol.auth.basic.grafana_cloud`'s `username` (the instance ID) and `otelcol.exporter.otlphttp.cloud`'s `endpoint` (the OTLP gateway URL) if they've changed from what's checked in.
...
**Point ca at Cloud too** (separate repo, `buritica/ca`): set `OTEL_EXPORTER_OTLP_ENDPOINT` and `OTEL_EXPORTER_OTLP_HEADERS` to the SAME OTLP gateway URL + Basic-auth header from step 1 above (the token is already scoped for this). This is independent of everything above — ca's telemetry never touches halfmoon's Alloy.
...
**Validated on halfmoon (2026-08-21), against real Alloy AND a real Grafana Cloud stack, not just written and hoped:** `alloy validate` passes, and a throwaway live run (real credentials, real network, real halfmoon containers) reported all 23 components healthy with zero export errors sustained over 35+ seconds. Two real bugs were caught and fixed in the process, not just a syntax check:
- **Stale-backlog flood**: `loki.source.docker` replays each container's FULL log history on startup (some going back months on halfmoon); Cloud's Loki rejects anything older than ~7 days with a 400, and the resulting flood of rejected old entries backed up the export queue and started dropping FRESH data too. Fixed with a `stage.drop { older_than = "24h" }` on every log pipeline (placed after `stage.timestamp` for the file-based ones, since `older_than` checks whatever timestamp is current at that point in the pipeline).
- **No batching**: without a batch processor, every metric point and log line became its own individual HTTP request — the exporter's default queue filled faster than individual requests could complete. Fixed with `otelcol.processor.batch` sitting between both receivers and the exporter.

Also switched from the classic per-product `prometheus.remote_write` + `loki.write` push paths (separate URL + instance ID per signal) to Grafana Cloud's unified OTLP gateway (`otelcol.receiver.prometheus` / `otelcol.receiver.loki` → `otelcol.processor.batch` → `otelcol.exporter.otlphttp`) — one endpoint, one credential, covers metrics + logs + traces together, and it's the same credential ca's own direct OTLP push will use. **Not yet validated:** `config/grafana-cloud/alert-rules.yml` / `alertmanager.yml` (need `mimirtool` against a real stack), and the exact label/field mapping showing up correctly in Cloud's Loki/Mimir once real data lands over a longer window — verify those before fully trusting the migration. Also not yet done: the actual halfmoon cutover (this was validated in a throwaway container, not deployed via `docker compose` yet) and decommissioning local Prometheus/Loki/Tempo/Alertmanager.
